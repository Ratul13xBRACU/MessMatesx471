...
def home():
    ...
    return render_template('home.html', user=user,current_month= datetime.now().strftime("%B"))
...
def profile():
    ...
    # Calculate meal rate: total_bazaar / total_meals_this_month
    ...
    # Calculate the total meal cost for the user
    total_meal_cost = meal_rate * total_meals

    # Calculate shared cost per user in the mess
    ...
    # Non-negative due (can't be negative, set to 0)
...
def admin():
    ...
    if request.method == 'POST':
        ...
        # No need to deduct balance here; shared cost per user is handled in the profile route
        # Just calculate the total bazaar and shared cost, no balance changes here.
